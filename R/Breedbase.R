...
BreedbaseRequestWizard <- function(conn, data_type, filters = list(), verbose = FALSE) {
  ...
  resp = httr::POST(paste(conn$base(), "ajax", "breeder", "search", sep="/"), body = body, encode = "multipart", httr::timeout(3600))
...
BreedbaseRequestFilterGeno <- function(conn, type, accessions = NULL, verbose = FALSE) {
  ...
  resp = httr::POST(url, body = list(accession_ids = paste(accessions, collapse=",")), encode="form", httr::timeout(3600))
  content = httr::content(resp)
  httr::warn_for_status(resp)
  if ( verbose ) {
    cat(
...
BreedbaseRequestListArchivedVCF <- function(conn, genotyping_protocol_id = NULL, genotyping_project_id = NULL, verbose = FALSE) {
  ...
  resp = httr::GET(url, query=list(genotyping_protocol_id=genotyping_protocol_id, genotyping_project_id=genotyping_project_id), httr::timeout(3600))
  content = httr::content(resp)
  httr::warn_for_status(resp)
